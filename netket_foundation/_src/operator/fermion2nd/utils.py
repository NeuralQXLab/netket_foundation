...
def currently_jitting() -> bool:
    """Best-effort check mirroring the classic tracer detection pattern."""

    if jax is None or jnp is None or _jax_core is None:
        return False

    try:
        return isinstance(jnp.array(1) + 1, _jax_core.Tracer)
    except Exception:  # pragma: no cover - defensive
        return False


def _is_tracer_like(value) -> bool:
    return _jax_core is not None and isinstance(value, _jax_core.Tracer)


def _safe_isclose_zero(value, *, atol: float | None = None) -> bool:
    if _is_tracer_like(value):
        return False
    try:
        if atol is None:
            return bool(np.isclose(value, 0.0))
        return bool(np.isclose(value, 0.0, atol=atol))
    except Exception:  # pragma: no cover - non-numeric objects
        return False
...
def _cast_scalar_like(value, dtype):
    ...
    try:
        item = arr.item()
    except ValueError:  # pragma: no cover - multi-element edge case
        item = arr
    return item, _safe_isclose_zero(item)
...
def _safe_allclose(a, b, *, atol: float) -> bool:
    ...
    if jnp is not None and isinstance(res, jnp.ndarray):
...
def _check_hermitian(
    ...
    """
    if any(_is_tracer_like(weight) for weight in weights):
        return False
...
def _canonicalize_input(
    ...
    if not _safe_isclose_zero(constant, atol=cutoff):
        terms = [()] + list(terms)
        weights = [constant] + weights

    dtype = canonicalize_dtypes(float, *weights, constant, dtype=dtype)

    weights = _cast_weights_like(weights, dtype)
...
def _remove_dict_zeros(d: dict, cutoff: float) -> dict:
    ...
    for key, value in d.items():
        if _safe_isclose_zero(value, atol=cutoff):
            continue
        result[key] = value
    return result
...
def _dict_compare(d1: dict, d2: dict, cutoff: float) -> bool:
    ...
    for key in d1_keys:
        if not _safe_allclose(d1[key], d2[key], atol=cutoff):
            return False
    return True
