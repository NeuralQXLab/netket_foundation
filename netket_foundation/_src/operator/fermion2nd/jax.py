...
def prepare_terms_list(
    ...
    res = []
    tracing = currently_jitting()
    for d in terms_dicts.values():
        weights_list = list(d.values())
        if tracing and any(_is_tracer_like(w) for w in weights_list):
            w = jnp.asarray(weights_list, dtype=weight_dtype)
        else:
            w = jnp.array(weights_list, dtype=weight_dtype)
