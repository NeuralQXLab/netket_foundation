...
class FermionOperator2ndBase(DiscreteOperator):
    ...
    def __iadd__(self, other):
        if _is_tracer_like(other):
            self_ops = self._operators
            updated = self_ops.get((), 0) + other
            if not _is_tracer_like(updated) and _safe_isclose_zero(updated):
                self_ops.pop((), None)
            else:
                self_ops[()] = updated
            self._reset_caches()
        ...
        for t, w in other._operators.items():
            ...
            if sw is None and not _safe_isclose_zero(w):
                self_ops[t] = w
            elif sw is not None:
                w = sw + w
                if _safe_isclose_zero(w):
                    del self_ops[t]
                else:
                    self_ops[t] = w
        self._reset_caches()
    ...
    def __imul__(self, scalar):
        treat_as_scalar = is_scalar(scalar) or _is_tracer_like(scalar)
        if not treat_as_scalar:
            # we will overload this as matrix multiplication
            return self._op__imatmul__(scalar)
        if not np.can_cast(_dtype(scalar), self.dtype, casting="same_kind"):
            raise ValueError(
                f"Cannot multiply inplace scalar with dtype {type(scalar)} "
                f"to operator with dtype {self.dtype}"
            )
        scalar, scalar_is_zero = _cast_scalar_like(scalar, self.dtype)
