...
def _multiply_by_pdf(oks, pdf):
    ...
    return jax.tree_util.tree_map(
        lambda x: jax.lax.broadcast_in_dim(pdf, x.shape, tuple(range(pdf.ndim))) * x,
        oks,
    )


@partial(jax.jit, static_argnames=("mode", "n_replicas"))
def _prepare_input(
    O_L,
    local_grad,
    *,
    ...
    Args:
        ...
        n_replicas: The number of replica chains.
        pdf: Optional probability density for the samples.
    ...
    local_grad_r = local_grad.reshape(n_replicas, -1)

    if pdf is None:
        O_L = O_L - jnp.mean(O_L, axis=1, keepdims=True)
        # center wrt mean
        de = local_grad_r - local_grad_r.mean(axis=-1, keepdims=True)
        de = de.flatten()
    ...
    if mode == "complex":
        ...
        # (nr, #ns, 2, np) -> (#ns*2*nr, np)
        O_L = jax.lax.collapse(O_L, 0, 3)
        # (#ns, 2) -> (#ns*2)
