    ...
    def setup(self):
        ...
        if self.is_equivariant is False:
            self.base_J = self.param(
                "J", init, (self.heads, self.n_patches, self.n_patches)
            )
            ...
            self.base_J = self.param(
                "J", init, (self.heads, sidex, self.n_patches), dtype=self.param_dtype
            )  # h matrices of size Lx_eff x Lx_eff x Ly_eff

            self.full_J = roll_vertical(
                self.base_J, jnp.arange(sidey), sidex, sidey
            ...
            self.full_J = roll2d(
                ...
                (sidex, sidey),
                self.graph,
            ).reshape(self.heads, self.n_patches, self.n_patches)

        else:
            raise ValueError(
                "You must provide either a graph for translational invariance or you must set is_translational_invariant to False"
            )
