...
class foundation_backflow(nn.Module):
    ...
    def fermi_sea(self, nf_i):
        ...
        HFS = jnp.zeros((self.hilbert.n_orbitals, self.hilbert.n_orbitals),
    ...
    def batch_spin_flip(self, n):
        ...
        if self.hilbert.n_fermions_per_spin[0] != self.hilbert.n_fermions_per_spin[1]:
            raise ValueError("Spin sectors must have the same number of fermions in order to enforce spin-flip symmetry.")
        ...
        couplings = n[:, 2 * N_orb:] # Parametri foundation (U, disordine, ecc.). Se vuoto, JAX lo gestisce senza errori.
...
class foundation_generalized_backflow(nn.Module):
    ...
    def __call__(self, n):
        ...
        def log_sdj(n_vec, F_vec):
            ...
            N_orb = self.hilbert.n_orbitals
            N_fermions = self.hilbert.n_fermions # Numero TOTALE di fermioni (UP + DOWN)
