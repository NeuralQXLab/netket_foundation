...
hi = nk.hilbert.SpinOrbitalFermions(N, s=1/2, n_fermions_per_spin=(N_fermions,N_fermions))
...
vit = foundation_ViT_trans_equi(n_layers=n_layers,
                                d_model=d_latent,
                                d_output=d_output,
                                d_latent=d_latent,
                                heads=heads,
...
backflow = foundation_backflow(model = vit,
                                hilbert = hi,
                                graph = graph,
                                param_dtype=pars_type
                                )
...
f_jastrow_mlp = foundation_fermi_Jastrow_MLP(n_layers=n_layers,
                                            n_coups=n_coups,
                                            d_model=d_latent,
...
parameter_array = jnp.linspace(0., 4., 50, dtype=pars_type).reshape(-1, 1)
n_replicas = parameter_array.shape[0]
ps = nkf.ParameterSpace(N=1, min=0., max=4.)

sa = nk.sampler.MetropolisFermionHop(hilbert=hi,
                                n_chains=n_replicas,
                                graph=graph,
                                sweep_size=hi.size,
                                )

vs = nkf.FoundationalQuantumState(sa, 
                                  J_multi, 
                                  ps, 
                                  n_replicas=n_replicas, 
                                  seed=seed, 
                                  n_samples=n_replicas*16
                                  )
